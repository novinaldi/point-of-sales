...
class Penjualan extends CI_Controller
{
    ...
    {
        if ($this->input->is_ajax_request() == true) {
            $this->load->model('penjualan/Modelsatuan', 'satuan');
    ...
    function tampildetailpenjualanbarang()
    {
        if ($this->input->is_ajax_request() == true) {
            ...
            JOIN satuan ON satid=produk.`produksatid` WHERE tempnota=? and tempuserinput = ? order by id desc";
    ...
    function updatepenjualan()
    {
        if ($this->input->is_ajax_request() == TRUE) {
            ...
            $pesan = ['sukses' => site_url('kasir/penjualan/cetakfaktur/' . $nota)];
    ...
    function ambilseluruhdatapenjualan()
    {
        if ($this->input->is_ajax_request() == true) {
            ...
            foreach ($list as $field) {
                ...
                $tombolcetak = "<button title=\"Cetak Faktur\" type=\"button\" class=\"btn btn-sm btn-circle btn-outline-info\" onclick=\"cetak('" . $field->jualnota . "')\"><i class=\"fa fa-fw fa-print\"></i></button>";
                $no++;
                $row = array();
                $row[] = $no;
                $row[] = $field->jualnota;
